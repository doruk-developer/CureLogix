...
{
    public class DiseaseValidator : AbstractValidator<DiseaseAddDto>
    {
        public DiseaseValidator()
        {
            RuleFor(x => x.Name)
                .NotEmpty().WithMessage("Hastalık adı zorunludur!");
            ...
            RuleFor(x => x.RiskLevel)
                .NotEmpty().WithMessage("Risk seviyesi seçilmelidir!");
        }
    }
}